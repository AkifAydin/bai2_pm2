        ...
        if (byContractType==null){
            Stream.of(ContractType.values()).map(ct -> {
                ...
                typeNode.getChildren().addAll(
                        contracts.stream().
                                filter(cont -> cont.getContractType()==ct).sorted(Comparator.comparing(Contract::getContractId)).
        ...
        if (byContractCustomer == null) {
            ...
            customers.stream().map(cust -> {
                ...
                custNode.getChildren().addAll(
                        contracts.stream().
                                filter(ct -> ct.getCustomer().equals(cust)).sorted(Comparator.comparing(Contract::getContractId)).
